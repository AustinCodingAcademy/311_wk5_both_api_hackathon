...
mysql -u root -h 104.198.67.226 -p
...
- Update the `getEmployeesById` function so that it calls the database, **selecting all fields from the employees table where the emp_no matches the id query parameter**. The should return one result. Use `res.json` to return the result to the user

- Update the `getEmployeesByFirstName` function so that it calls the database, **selecting all fields from the employees table where the first_name matches the first_name query parameter**. The could return multiple results. Use `res.json` to return the results to the user
